...
public:
    TEST_METHOD(findCommandTest) {
        ...
        std::string obj = tokenizer.findCommand(inputArr);
    ...
    TEST_METHOD(findTagsTest) {
        ...
        std::vector<std::string> obj = tokenizer.findTags(ParserLibrary::explode(input, " "));
    ...
    TEST_METHOD(findDescriptionTest) {
        ...
        std::string obj = tokenizer.findDescription(ParserLibrary::explode(input, " "));
        Assert::AreEqual(expected, obj);
    }
    ...
    //    DateTimeToken* obj = tokenizer.findDate(ParserLibrary::explode(input, " "));
